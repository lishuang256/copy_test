...
CMD
    ...
    cat <<'CMD'
(2) 测试Refund交易, 结果为False: ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Refund", "Args": ["101facemask", "100000facemask1092124483650522049705920057327600590256125547533636349276525362601168988481396265766022768170794111142310884989615889545183642865508508811539477701719982100111112025-10-01T12:01:01"]}' --waitForEvent
CMD
    pause_for_enter "按回车执行第(2)条交易..."
    ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Refund", "Args": ["101facemask", "100000facemask1092124483650522049705920057327600590256125547533636349276525362601168988481396265766022768170794111142310884989615889545183642865508508811539477701719982100111112025-10-01T12:01:01"]}' --waitForEvent
    pause_for_enter "第(2)条交易已返回结果，按回车继续..."

    cat <<'CMD'
(3) 测试Refund交易, 结果为True: ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Refund", "Args": ["100facemask", "100100facemask1092124483650522049705920057327600590256125547533636349276525362601168988481396265766022768170794111142310884989615889545183642865508508811539477701719982100111112025-10-01T12:01:01"]}' --waitForEvent
CMD
    pause_for_enter "按回车执行第(3)条交易..."
    ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Refund", "Args": ["100facemask", "100100facemask1092124483650522049705920057327600590256125547533636349276525362601168988481396265766022768170794111142310884989615889545183642865508508811539477701719982100111112025-10-01T12:01:01"]}' --waitForEvent
